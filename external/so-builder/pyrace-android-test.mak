...
$CPP $_LDOPT -o IOManager_lib.so IOManager_wrap.o $_OBJ_LIST -L. libjsoncpp.so -L. CLICodec_lib.so -Wl,-rpath,/data/data/com.twosix.race/race/comms/PluginCOMMSSRIPixelfed/libs -Wl,-rpath,/system/lib

_make_CLICodec -L. libjsoncpp.so -L. IOManager_lib.so -Wl,-rpath,/data/data/com.twosix.race/race/comms/PluginCOMMSSRIPixelfed/libs -Wl,-rpath,/system/lib # make CLICodec_lib.so that references IOManager_lib.so
