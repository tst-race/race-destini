...
from mcgroups import *

# Local Lib Imports
#jj is testing this -> remove me
...
def main() -> None:
    """
    Purpose:
        Generate configs for twoSixIndirectCpp
    Args:
        N/A
    Returns:
        N/A
...
def generate_configs(
    ...
    user_responses = generate_user_responses(range_config, runmode)
...
def generate_genesis_link_addresses(
    ...
    tagSeed     = str (uuid.uuid1 ())

    if local_override:
        wbname = "race.example2"
    else:
        wbname = "pixelfed-destinicomms.test"
    ...
    base_client_hashtag = "python_clients_{link_idx}"
    base_server_hashtag = "python_servers_{link_idx}"
    for link_idx, requested_link in enumerate(network_manager_request["links"]):
        ...
        recipients = requested_link["recipients"]

        # ignore multicast links
        if len(recipients) > 1:
            continue
        ...
        if len(recipients) > 1:
            recipient = gen_mc_group(recipients)
        else:
            recipient = recipients[0]

        rtag = recipient + tagSeed
        ...
        link_addresses[sender_node].append(
            {
                ...
                "address": f'{{"persona": "{recipient}", "tag_seed": "{rtag}", "whiteboards": ["{wbname}"], "s_persona": "{recipient}", "s_tag_seed": "{rtag}", "s_whiteboards": ["{wbname}"], "startTransport": "True"}}',
        ...
        link_addresses[recipient].append(
        {
            ...
            "address": f'{{"persona": "{sender_node}", "tag_seed": "{rtag}", "whiteboards": ["{wbname}"], "s_persona": "{recipient}", "s_tag_seed": "{rtag}", "s_whiteboards": ["{wbname}"], "startTransport": "True"}}',
...
def build_link_properties() -> Tuple[Dict[str, Any], Dict[str, Any]]:
    ...
    # Build link properties
    link_bandwidth_bps = 15_000  # tested with rib local mode against v1.0.0
    link_latency_ms = 3_300  # tested with rib local mode against v1.0.0
...
def generate_user_responses(
    range_config: Dict[str, Any], runmode
