...
# Start port for python exemplar
START_PORT = 22_000
END_PORT = 23_999
...
def main() -> None:
    """
    Purpose:
        Generate configs for twoSixDirectCpp
    Args:
        N/A
    Returns:
        N/A
...
def generate_configs(
    ...
    # Store the resulting configs (link-profiles.json for exemplar plugins)
    file_utils.write_json(
        ...
        for sender_port, sender_nodes in sender_port_mapping.items():
            ...
            for sender_node in sender_nodes:
                ...
                link_addresses[recipient_node].append(
                    {
                        ...
                        "address": f'{{"persona": "{sender_node}", "tag_seed": "{rtag}", "whiteboards": ["{wbname}"], "s_persona": "{recipient_node}", "s_tag_seed": "{rtag}", "s_whiteboards": ["{wbname}"], "startTransport": "True"}}',
                ...
                link_profile = {
                    ...
                    "address": f'{{"persona": "{recipient_node}", "tag_seed": "{rtag}", "whiteboards": ["{wbname}", "{sender_node}"], "s_persona": "{recipient_node}", "s_tag_seed": "{rtag}", "s_whiteboards": ["{wbname}", "{sender_node}"], "startTransport": "True"}}',
...
def build_link_properties() -> Tuple[Dict[str, Any], Dict[str, Any]]:
    ...
    # Build link properties
    link_bandwidth_bps = 300_000  # tested with rib local mode against v1.0.0
    link_latency_ms = 4000  # tested with rib local mode against v1.0.0
